...
def model(folder_path, out_classes):
    ...
    old_model = torchvision.models.googlenet(pretrained=True)
    ...
    class whole_cnn(nn.Module):
        def __init__(self):
            ...
            self.add_linear = torch.nn.Sequential(nn.Linear(1024,out_classes,bias=True))
    ...
    def train_model(model, criterion, optim, epoch_no):
        ...
        for epoch in range(epoch_no):
            train_features = np.zeros((50,1024))
            train_labels = []
            running_loss = 0.0
            running_acc = 0.0
            model.train()

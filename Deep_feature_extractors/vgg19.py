...
def model(folder_path, out_classes):
    ...
    old_model = torchvision.models.vgg19(pretrained=True)
    ...
    class whole_cnn(nn.Module):
        def __init__(self):
            ...
            self.remv_classifier = torch.nn.Sequential(*(list(old_model.children())[:-1]))

            # Re-assigning the removed layers of VGG-19 and storing it in our own CNN to find change in accuracy

            self.flatten = nn.Flatten()
            self.add_classifier = torch.nn.Sequential(nn.Linear(25088,4096,bias=True),
                                                nn.ReLU(),
                                                nn.Dropout(0.5),
        ...
        def forward(self,x):
            output = self.remv_classifier(x)
            output = self.flatten(output)
            x_deep = output
            output_new = self.add_classifier(output)
    ...
    def train_model(model, criterion, optim, epoch_no):
        ...
        for epoch in range(epoch_no):
            train_features = np.zeros((50,25088))
            train_labels = []
            running_loss = 0.0
            running_acc = 0.0
            model.train()
    ...
    def model_val(model, criterion, optim):
        ...
        running_val_acc = 0.0
        val_features = np.zeros((30,25088))
        val_labels = []
